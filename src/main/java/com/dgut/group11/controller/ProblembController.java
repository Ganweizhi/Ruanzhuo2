...
import com.dgut.group11.service.ProblembService;
...
@RestController
@CrossOrigin(origins = "*")
public class ProblembController {
    ...
    @Autowired
    private ProblembService problembService;
...
//    }
    ...
    @RequestMapping(value="/jiao_xue_nan_dian/{success_id}",method = {RequestMethod.POST})
    public String jiao_xue_nei_rong(@PathVariable("success_id") String coureseId){
        ...
        List<Problemb> nanDianList = problembService.findAllProblembBySuccessCourseId(success_id);
        JiaoXueNanDian jiaoXueNanDian = new JiaoXueNanDian(successCourse,nanDianList);
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("data",jiaoXueNanDian);
