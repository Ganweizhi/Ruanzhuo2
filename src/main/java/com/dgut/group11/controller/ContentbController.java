...
@RestController
@CrossOrigin(origins = "*")
public class ContentbController {
...
//    }
    ...
    @RequestMapping(value="/jiao_xue_nei_rong/{success_id}",method = {RequestMethod.POST})
    public String jiao_xue_nei_rong(@PathVariable("success_id") String coureseId){
        ...
        List<Contentb> neiRongList = contentbService.findAllContentbBySuccessCourseId(success_id);

        JiaoXueNeiRong jiaoXueNeiRong = new JiaoXueNeiRong(successCourse,neiRongList);
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("data",jiaoXueNeiRong);
